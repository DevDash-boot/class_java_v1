...
public class FileClient {
    ...
    public static void main(String[] args) {
        ...
        String filePath = "C:\\jhg\\work_space\\java_class_1\\java_v1\\assets\\a.txt";
        ...
        // 클라이언트 입장에서는 서버 측에 경로를 제외하고 파일명만 보내야한다.
        // C:\work_java\test.txt --> test.txt만 경로에서 걸러내야 한다.
        // 즉 경로를 통째로 보내면 서버가 엉뚱한 위치에 저장하게 되므로 이름만 보낸다.(물론 방어적 코드로 작성되어 있음)
        String fileName = file.getName();
        byte[] nameBytes = fileName.getBytes();

        // 이름의 길이를 1바이트에 담아 보내므로 255를 넘으면 안된다.
        // 한글은 UTF-8 기준에서 한 글자가 3바이트라서 최대 85글자가 정도가 한계이다.
        if (nameBytes.length > 255) {
            System.out.println("파일 이름이 너무 깁니다. (최대 255 바이트)");
            return;
        }
        ...
        try (Socket socket = new Socket(HOST, PORT)) {
            ...
            // 소켓의 나가는 방향만 닫는다.
            // 다 종료해버리면 들어오는 스트림도 닫혀서 응답을 받을 수 없다.
